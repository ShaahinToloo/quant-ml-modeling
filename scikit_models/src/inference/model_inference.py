...
class ModelInference:
    def __init__(
        ...
        if self.peak_model_obj is not None:
            self.peak_stdscaler = CustomStandardScaler("resources/scalers")
            self.peak_stdscaler.load_scaler_from_json("mlp_c_b_peak_std_scaler.json")

        self.trough_stdscaler = None
        if self.trough_model_obj is not None:
            self.trough_stdscaler = CustomStandardScaler("resources/scalers")
            self.trough_stdscaler.load_scaler_from_json(
                "mlp_c_b_trough_std_scaler.json"
            )
        ...
        for i in range(self.seq_len, len(self.df) - self.look_ahead):
            if self.df.iloc[i]["fracHigh7"] == 1.0:
                ...
                self.peak_y[i] = (
                    1
                    if (
                        self.peak_model_obj.predict_proba(sequence) >= self.threshold
                    )
                    else 0
                )
        ...
        for i in range(self.seq_len, len(self.df) - self.look_ahead):
            if self.df.iloc[i]["fracLow7"] == 1.0:
                ...
                self.trough_y[i] = (
                    1
                    if (
                        self.trough_model_obj.predict_proba(sequence) >= self.threshold
                    )
                    else 0
                )
