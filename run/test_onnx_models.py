...
print(f"Output (batch=1024): shape={out1024.shape}, first 3 batches predictions ={out1024[:3]}")

X10 = np.random.rand(10, 920).astype(np.float32)
out10 = session.run([output_name], {input_name: X10})[0]
print(f"Output (batch=10): shape={out10.shape}, first 3 batches predictions ={out10[:3]}")
